...
const empty = { stepNo: '', title: '', description: '', sortOrder: 0, isActive: true };
...
export default function NewWorkingProcessPage() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      await adminApi.createWorkingProcess({ ...form, stepNo: Number(form.stepNo), sortOrder: Number(form.sortOrder) });
  ...
  return (
    <div className="max-w-xl">
      <h1 className="text-2xl font-bold mb-6">New Process Step</h1>
      <WorkingProcessForm form={form} onChange={handleChange} onSubmit={handleSubmit} error={error} loading={loading} />
    </div>
  );
}

export function WorkingProcessForm({ form, onChange, onSubmit, error, loading }) {
  return (
    <form onSubmit={onSubmit} className="space-y-4 bg-white p-6 rounded-lg border">
      <div>
        ...
        <input type="number" name="stepNo" value={form.stepNo} onChange={onChange} required className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-black" />
      </div>
      <div>
      ...
      <div className="flex gap-3">
        ...
        <a href="/admin/working-processes" className="px-6 py-2 rounded border hover:bg-gray-50 transition">Cancel</a>
      </div>
    </form>
  );
}